...
1. Откройте portable-папку `TeleVault-v2.8.9` в Проводнике Windows.
...
`TeleVault.exe` в 2.8.9 - основной пользовательский способ запуска portable-папки. Это не installer и не one-file exe.
...
2. Получите архив `dist\TeleVault-v2.8.9.zip`.
3. Распакуйте zip в обычную папку.
4. Откройте распакованную папку `TeleVault-v2.8.9`.
...
Zip содержит верхнюю папку `TeleVault-v2.8.9\`, а внутри неё те же файлы, что и clean portable folder. Если zip собран через `build_exe_launcher.bat`, внутри верхней папки также есть `TeleVault.exe`. Если в исходном проекте есть `runtime\python\python.exe`, bundled Python попадёт и в zip. Если runtime отсутствует, zip всё равно создаётся, но для запуска на другой машине понадобится установленный `py` или `python`.
...
и проверяет, что `TeleVault-v2.8.9/TeleVault.exe` есть внутри zip.
...
Начиная с 2.8.9 builder перед компиляцией проверяет, что package version, `app.py APP_VERSION` и launcher `AppVersion` совпадают. Если версии расходятся, сборка падает с понятной ошибкой до создания `TeleVault.exe`, чтобы portable package не получил launcher со старой expected version.

Репозиторий содержит `assets\TeleVault.ico`, поэтому builder передаёт его в `csc.exe` через `/win32icon`, и `TeleVault.exe` собирается с dedicated TeleVault icon. Если файла нет, сборка не падает и печатает warning, что custom icon не найден, а exe будет собран со стандартной Windows-иконкой.
