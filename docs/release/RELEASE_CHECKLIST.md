...
TeleVault X.Y.Z - общий checklist для подготовки Windows-комплекта, отдельного Windows 7 legacy package при необходимости и ручной проверки перед публикацией. Это не installer и не one-file exe: launcher лежит рядом с `app.py`, `assets/`, `backend/`, `frontend/` и выбранным bundled runtime. Обычный пользовательский запуск portable-версии: скачать zip, распаковать папку и запустить `TeleVault.exe`.
...
`TeleVault.exe` запускает `runtime\python\pythonw.exe app.py` из папки, где лежит сам exe, без видимой консоли, ждёт готовности сервера и открывает app-like окно через Edge/Chrome или обычный browser fallback. `run_windows.bat` остаётся запасным/dev-способом запуска для разработки и диагностики: он сначала пробует `runtime\python\python.exe`, затем системный `py`, затем системный `python`, и может открывать интерфейс иначе, чем launcher-версия.
...
Это основной пользовательский путь запуска portable-версии. Проверку иконки приложения, taskbar/start behavior, повторного запуска и остановки backend после закрытия окна выполнять через `TeleVault.exe`, а не через `run_windows.bat`.
...
## Launcher UX and taskbar checks
...
6. Убедиться, что `TeleVault.exe` показывает TeleVault icon в taskbar/start context, если Windows принимает launcher identity.
7. Закрыть app-like окно при живом backend, запустить `TeleVault.exe` снова и убедиться, что открывается одно новое app-like окно с сохранённым размером/позицией.
8. Убедиться, что `user_data\launcher_window.json` локальный, игнорируется git и не попадает в zip/source release.
9. Убедиться, что `logs\launcher.log` содержит события loaded/no/invalid/saved window state и owner hwnd found/not found без export paths и private message data.

## Fallback run_windows.bat

`run_windows.bat` — запасной/dev-способ запуска для разработки и диагностики. Он должен оставаться рабочим, но его не использовать для проверки `TeleVault.exe` launcher identity, taskbar/start icon, повторного запуска или закрытия owned backend.
...
6. Убедиться, что `run_windows.bat` в распакованной копии тоже работает как fallback/dev-запуск.
